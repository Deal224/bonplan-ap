export function Input({ label, type = 'text', value, onChange, placeholder, error, prefix, suffix, className = '', ...props }) {
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      {label && <label className="text-sm font-medium text-[#1A4731] dark:text-slate-300">{label}</label>}
      <div className="relative flex items-center">
        ...
        <input
          ...
          className={`
            w-full py-3 px-4 rounded-2xl border bg-white dark:bg-slate-700 text-[#1A4731] dark:text-white text-sm
            placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none transition-all
            focus:border-[#1A4731] focus:ring-2 focus:ring-[#1A4731]/10
            ${error
