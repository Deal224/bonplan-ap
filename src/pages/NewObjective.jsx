...
const RULE_TYPES = [
  { value: 'none', labelFr: 'Aucune restriction', labelEn: 'No restriction', icon: '🔓' },
  { value: 'max_monthly', labelFr: 'Retrait max mensuel', labelEn: 'Monthly max withdrawal', icon: '📅' },
  { value: 'min_balance', labelFr: 'Solde minimum', labelEn: 'Minimum balance', icon: '🛡️' },
  { value: 'max_amount', labelFr: 'Montant max par retrait', labelEn: 'Max per withdrawal', icon: '💎' },
];
...
export default function NewObjective() {
  ...
  const validateStep1 = () => {
    ...
    if (!form.target_amount || Number(form.target_amount) < 1000) errs.target_amount = 'Minimum 1 000 GNF';
    if (!form.lock_date) errs.lock_date = 'Date requise';
    else if (new Date(form.lock_date) <= new Date()) errs.lock_date = 'La date doit être future';
    setErrors(errs);
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">{T('newObjective')}</h1>
          <p className="text-slate-400 text-sm">Étape {step}/2</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {[1, 2].map(s => (
          <div
            key={s}
            className={`h-1 flex-1 rounded-full transition-all ${s <= step ? 'bg-[#1A3C6E]' : 'bg-slate-200'}`}
          />
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 1 ? (
          ...
          >
            <div>
              <h2 className="text-lg font-bold text-slate-900 mb-1">{T('withdrawalRule')}</h2>
              <p className="text-sm text-slate-400 mb-4">
                Définissez des règles pour vous protéger contre les retraits impulsifs
              </p>
                  ...
                  >
                    ...
                    <div>
                      <p className={`font-semibold text-sm ${form.rule_type === rule.value ? 'text-[#1A3C6E]' : 'text-slate-900'}`}>
                        {state.lang === 'en' ? rule.labelEn : rule.labelFr}
                      </p>
                    </div>
                    {form.rule_type === rule.value && (
              ...
              {form.rule_type !== 'none' && (
                ...
                >
                  <Input
                    label={
                      form.rule_type === 'max_monthly' ? 'Montant max par mois (GNF)' :
                      form.rule_type === 'min_balance' ? 'Solde minimum à maintenir (GNF)' :
                      'Montant max par retrait (GNF)'
                    }
                    type="number"
            ...
            <div className="bg-slate-50 rounded-2xl p-4">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Récapitulatif</p>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{Number(form.target_amount).toLocaleString('fr-FR')} GNF</p>
                </div>
                <div>
                  <p className="text-slate-400">Verrouillé jusqu'au</p>
                  <p className="font-semibold">{new Date(form.lock_date).toLocaleDateString('fr-FR')}</p>
                </div>
              </div>
            </div>
