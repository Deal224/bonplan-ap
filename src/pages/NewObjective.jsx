...
export default function NewObjective() {
  ...
  const [form, setForm] = useState({
    name: '', emoji: '🎯', target_amount: '', lock_date: '', rule_type: 'none', rule_value: '',
  });
  ...
  return (
    <div className="flex flex-col gap-6 max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#1A4731] dark:text-white">{T('newObjective')}</h1>
      ...
      <div className="flex gap-2">
        {[1, 2].map(s => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-all ${s <= step ? 'bg-[#FFBE00]' : 'bg-slate-200 dark:bg-slate-700'}`} />
        ))}
      </div>

      <AnimatePresence mode="wait">
        {step === 1 ? (
          <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="flex flex-col gap-5">
            {/* Emoji picker */}
            <div>
              <label className="text-sm font-medium text-[#1A4731] dark:text-slate-300 block mb-2">{T('icon')}</label>
                ...
                {emojiList().map(e => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-xl flex items-center justify-center transition-all cursor-pointer ${
                      form.emoji === e
                        ? 'bg-[#FFBE00] scale-110 shadow-md'
                        : 'bg-slate-100 dark:bg-slate-700 hover:bg-[#FFF8E1] dark:hover:bg-slate-600'
                    }`}
                  >
                    {e}
                  </button>
            ...
            <Input label={T('objectiveName')} value={form.name} onChange={e => set('name', e.target.value)} placeholder="ex: Vacances, Loyer..." error={errors.name} />
            <Input label={T('targetAmount')} type="number" value={form.target_amount} onChange={e => set('target_amount', e.target.value)} placeholder="500 000" suffix="GNF" error={errors.target_amount} />
            <Input label={T('lockDate')} type="date" value={form.lock_date} onChange={e => set('lock_date', e.target.value)} error={errors.lock_date} min={new Date(Date.now() + 86400000).toISOString().split('T')[0]} />
            <Button fullWidth onClick={() => { if (validateStep1()) setStep(2); }}>{T('continue')}</Button>
          </motion.div>
        ) : (
          <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="flex flex-col gap-5">
            <div>
              <h2 className="text-lg font-bold text-[#1A4731] dark:text-white mb-1">{T('withdrawalRule')}</h2>
              ...
              <div className="flex flex-col gap-3">
                {RULE_TYPES.map(rule => (
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition-all cursor-pointer ${
                      form.rule_type === rule.value
                        ? 'border-[#FFBE00] bg-[#FFF8E1] dark:bg-yellow-900/20'
                  ...
                  >
                    ...
                    <div>
                      <p className={`font-semibold text-sm ${form.rule_type === rule.value ? 'text-[#1A4731]' : 'text-slate-900 dark:text-white'}`}>
                        {T(rule.labelKey)}
                      </p>
                    </div>
                    {form.rule_type === rule.value && (
                      <div className="ml-auto w-5 h-5 rounded-full bg-[#FFBE00] flex items-center justify-center">
                        <svg className="w-3 h-3 text-[#1A4731]" fill="currentColor" viewBox="0 0 20 20">
            ...
            {/* Summary */}
            <div className="bg-[#FFF8E1] dark:bg-slate-700/50 rounded-2xl p-4">
              <p className="text-xs font-semibold text-[#1A4731]/60 uppercase tracking-wide mb-3">{T('summary')}</p>
              <div className="flex gap-2 items-center mb-2">
                <span className="text-2xl">{form.emoji}</span>
                <span className="font-bold text-[#1A4731] dark:text-white">{form.name}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <p className="text-slate-400">{T('target')}</p>
                  <p className="font-semibold text-[#1A4731] dark:text-white">
            ...
            <Button fullWidth loading={loading} onClick={handleSubmit}>🎯 {T('createObjective')}</Button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
