...
export default function Profile() {
  ...
  async function handleChangePin() {
    if (pins.newPin !== pins.confirm) { toast.error(T('pinMismatch')); return; }
    if (pins.newPin.length !== 4) { toast.error(T('pinLength')); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="flex flex-col gap-5 max-w-lg mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-[#1A4731] dark:text-white">{T('profile')}</h1>
      </div>

      {/* User card — gradient vert */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-gradient-to-br from-[#1A4731] to-[#0f2f1a] rounded-3xl p-6 text-white"
      >
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#FFBE00] flex items-center justify-center text-2xl font-bold text-[#1A4731]">
            {avatar}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xl font-bold truncate">{state.user?.name}</p>
            <p className="text-white/60 text-sm">{state.user?.phone}</p>
            <span className="inline-block bg-[#FFBE00]/20 text-[#FFBE00] text-xs px-2.5 py-1 rounded-full mt-1 capitalize font-medium">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
          ...
          {nameEdit ? (
            <div className="p-4 flex flex-col gap-3">
              <Input label={T('nameLabel')} value={name} onChange={e => setName(e.target.value)} placeholder="Votre nom" />
              <div className="flex gap-2">
                <Button variant="ghost" onClick={() => setNameEdit(false)} className="flex-1">{T('cancel')}</Button>
                <Button loading={loading} onClick={handleSaveName} className="flex-1">{T('save')}</Button>
              </div>
            </div>
          ) : (
            <button
              onClick={() => { setName(state.user?.name || ''); setNameEdit(true); }}
              className="w-full flex items-center justify-between px-4 py-4 hover:bg-[#FFF8E1] dark:hover:bg-slate-700 cursor-pointer transition-colors"
            >
              <div className="flex items-center gap-3">
                <span className="text-xl">👤</span>
                <div className="text-left">
                  <p className="text-sm font-medium text-[#1A4731] dark:text-white">{T('nameLabel')}</p>
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-4 hover:bg-[#FFF8E1] dark:hover:bg-slate-700 cursor-pointer transition-colors border-t border-slate-100 dark:border-slate-700"
          >
            <div className="flex items-center gap-3">
              <span className="text-xl">🔐</span>
              <p className="text-sm font-medium text-[#1A4731] dark:text-white">{T('changePin')}</p>
            </div>
            <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/>
            </svg>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-[#1A4731] dark:text-white">{T('darkMode')}</p>
            </div>
            <button
              onClick={() => dispatch({ type: 'SET_DARK', value: !state.darkMode })}
              className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${state.darkMode ? 'bg-[#1A4731]' : 'bg-slate-200'}`}
            >
          ...
          <div className="flex items-center justify-between px-4 py-4">
            <div className="flex items-center gap-3">
              <span className="text-xl">🌍</span>
              <p className="text-sm font-medium text-[#1A4731] dark:text-white">{T('language')}</p>
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
          ...
          <div className="px-4 py-4 flex items-center gap-3">
            <span className="text-xl">💰</span>
            <div>
              <p className="text-sm font-medium text-[#1A4731] dark:text-white">BON PLAN</p>
        ...
        <div className="flex flex-col gap-5">
          {pinStep === 1 && (
            <>
              ...
              <Button fullWidth onClick={() => { if (pins.current.length === 4) setPinStep(2); else toast.error(T('pinLength')); }}>{T('continue')}</Button>
            </>
          )}
          {pinStep === 2 && (
            <>
              ...
              <Button fullWidth onClick={() => { if (pins.newPin.length === 4) setPinStep(3); else toast.error(T('pinLength')); }}>{T('continue')}</Button>
            </>
          )}
          {pinStep === 3 && (
            <>
